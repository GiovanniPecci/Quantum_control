...
def optimize_pulse(fig_of_merit, theta_i, args, ftol, method, Emin, Emax, ORDER, HESS = False, verbose = True):
...
def generate_instance(w_file_path, char):
    ...
    params_dir = instance_dir+"/CRAB/"
    figures_dir = instance_dir+"/figs"

    output_dir = params_dir+'/test/'
...
def CRAB_main(qaoa_cirq, output_dir, figures_dir, tau, dt, basis, Ncs, n_reals = None, Full = False, RAND = True, ORDER = 1, Warm_start = False, Warm_from_file = False, input_file = None, opt_method = 'BFGS-analytical', HESS = False):
...
#    if Full: Ncs = [P]
    ...
    for Nc in Ncs:
        ...
        if RAND:
            ...
            if HESS:
                reals_hess = np.zeros((n_reals, 2*Nc+1, 2*Nc+1 ))
            ...
            for j in range(n_reals):
                ...
                if HESS:
                    reals_eres[j], reals_theta[j], reals_hess[j]= optimize_pulse(fig_of_merit, theta_start, args, ftol, method = opt_method, Emin = Emin, Emax = Emax, ORDER = ORDER, HESS = HESS, verbose = False)
                
                else:
                    reals_eres[j], reals_theta[j] = optimize_pulse(fig_of_merit, theta_start, args, ftol, method = opt_method, Emin = Emin, Emax = Emax, ORDER = ORDER, HESS = HESS, verbose = False)
            ...
            if Warm_start:
                filename_output = output_dir+'/_test_Warm_start_'+basis+'_OUTPUT_file_tau_'+str(tau)+"_dt_"+str(dt)+"Nc"+str(Nc)+"_nreals_"+str(n_reals)+".dat"
            else:  
                filename_output = output_dir+'/_test_'+basis+'_OUTPUT_file_tau_'+str(tau)+"_dt_"+str(dt)+"Nc"+str(Nc)+"_nreals_"+str(n_reals)+".dat"
        ...
        # RAND = FALSE
            ...
            if Warm_start:
                filename_output = output_dir+'_test_/Warm_start_'+basis+'_OUTPUT_file_tau_'+str(tau)+"_dt_"+str(dt)+"Nc"+str(Nc)+".dat"
            else: 
                filename_output = output_dir+'/_test_'+basis+'_OUTPUT_file_tau_'+str(tau)+"_dt_"+str(dt)+"Nc"+str(Nc)+".dat"
